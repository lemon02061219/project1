...
// 这个函数一样是调用了汇编指令，把标志寄存器的东西放到处理器栈上，
// 然后把值pop到flags（代表标志寄存器IF位）上，通过判断flags来返回当前中断状态(intr_level)。
enum intr_level
intr_get_level (void)
{
  uint32_t flags;
...
intr_enable (void)
{
  ...
  // intr_context()直接返回了是否外中断的标志in_external_intr， 
  // 就是说ASSERT断言这个中断不是外中断（IO等， 也称为硬中断）
...
//这里很明显，intr_level代表能否被中断，而intr_disable做了两件事情：
// 1. 调用intr_get_level()
// 2. 直接执行汇编代码，调用汇编指令来保证这个线程不能被中断。
enum intr_level
intr_disable (void)
{
  enum intr_level old_level = intr_get_level ();  // intr_get_level返回了intr_level的值

  /* Disable interrupts by clearing the interrupt flag.
     See [IA32-v2b] "CLI" and [IA32-v3a] 5.8.1 "Masking Maskable
     Hardware Interrupts". */
  // 注意： 这个asm volatile是在C语言中内嵌了汇编语言，调用了CLI指令，
  // CLI指令不是command line interface, 而是clear interrupt, 
  // 作用是将标志寄存器的IF（interrupt flag）位置为0, IF=0时将不响应可屏蔽中断。
